...
class NotificationController extends GetxController {
  static NotificationController get instance =>
      Get.isRegistered() ? Get.find() : Get.put(NotificationController());
    ...
    try {
      ...
      if (selectedNotification.value.id.isEmpty) {
        if (selectedNotificationId.isEmpty) {
          ...
          selectedNotification.value = await repository.fetchSingleItem(
  ...
  void listenToNotifications() {
    _notificationsSubscription = repository.fetchAllItemsAsStream().listen(
      (notificationList) {
        notifications.value =
            notificationList; // Update the notifications list in real-time
      },
      onError: (error) {
  ...
  void onClose() {
    _notificationsSubscription
        ?.cancel(); // Cancel the subscription when the widget is destroyed
    super.onClose();
  }
}
